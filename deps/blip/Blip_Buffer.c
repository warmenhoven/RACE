      ...
      long rate)
{
   ...
   /* ratio is always positive, so a truncating cast equals floor() here; this
    * avoids a libm dependency (the host core links with an empty LIBS). */
   blip_s64 factor = (blip_s64) (ratio * (1LL << BLIP_BUFFER_ACCURACY) + 0.5);
